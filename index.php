...
<head></head>
	...
	<div id="chartdiv"></div>
<?php
	$db = new SQLite3('/opt/cowrie/cowrie.db');
...
//------------------------------------------------------------------------------
	...
	$sql = "SELECT COUNT(endtime) AS total FROM sessions"; 
	$result = $db->query($sql);
	...
	echo '	<th class="text-center">Connection transport</th>';
	echo '	<th class="text-center">Connection protocol</th>';
	...
	$Index = 0;
	$cons = $db->query("SELECT endtime, starttime, ip FROM sessions ORDER BY endtime DESC LIMIT $start_from, $results_per_page");
	while($row = $cons->fetchArray(SQLITE3_ASSOC)) {
		$result = json_decode(file_get_contents("http://reallyfreegeoip.org/json/" . $row['ip']));
		$SmallArray[$Index] = array(
		 ...
		 $Index++;
		echo '<tr>';
		echo '	<td class="text-center">' . $row['starttime'] .'</td>';
		echo '	<td class="text-center">' . $row['endtime'] .'</td>';
		echo '	<td class="text-center">TCP</td>';
		echo '	<td class="text-center">SSH</td>';
		...
		echo '  <td class="text-center">' . $result->city . '</td>';
		echo '</tr>';
	}
	echo '</table>';
	echo '</div>';
	...
	echo '&laquo;</a></li>';
	 ...
	 if ($total_pages < 3) {
                for ($i=1; $i<= $total_pages; $i++) {  // print links for all pages
                    ...
                    echo "<a href='index.php?page={$i}'".' class="btn btn-default">'.$i."</a></li>";
                };
...
$('#form').submit(function () {
    $.post('replay.php', $('#form').serialize(), function (data, textStatus) {
         $('#replay').append(data);
    });
    return false;
});
